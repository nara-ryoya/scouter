...
analog_font_path = "BitterPro-Bold.ttf"
...
class FaceThread(threading.Thread):
	...
	def run(self):
            ...
            analog_font = ImageFont.FreeTypeFont(analog_font_path, size=self.fontSize)
            analog_small_font = ImageFont.FreeTypeFont(analog_font_path, size=self.fontSize // 2)
                    ...
                    for fx, fy, fw, fh in faces:
                        ...
                        if rec_mode:
                            ...
                            if len(face_landmarks) != 0:
                                ...
                                eye_left_w = distance(face_landmarks[42], face_landmarks[45])
                                ...
                                left_eye_vector = np.array(face_landmarks[36]) - np.array(face_landmarks[39])
                                right_eye_vector = np.array(face_landmarks[45]) - np.array(face_landmarks[42])
                                nose_vector = np.array(face_landmarks[30]) - np.array(face_landmarks[27])
                                                
                                left_eye_theta = np.sum(left_eye_vector * nose_vector) /(np.linalg.norm(left_eye_vector) * np.linalg.norm(nose_vector))
                                right_eye_theta = np.sum(right_eye_vector * nose_vector) /(np.linalg.norm(right_eye_vector) * np.linalg.norm(nose_vector))
                                
                                theta = left_eye_theta + right_eye_theta
                                theta /= 2
                                ...
                                x = width//2 - (analog_font.getsize(word)[0] // 2)
                                y = height//2 - int(self.fontSize)
                                y = max(y, 0)
                                draw.text((x, y), word, font = analog_font, fill = textColor)

                                x = width // 2 - (font.getsize(str(tmp_strongness))[0] // 2)
                                y +=  int(self.fontSize) * 2
            ...
            left_upper_text = "strongness:"
            img_pil = Image.fromarray(tmp)
            draw = ImageDraw.Draw(img_pil)
            x = width//30
            y = height//8
            y = max(y, 0)
            draw.text((x, y), left_upper_text, font=analog_small_font, fill=textColor)

            x += analog_small_font.getsize(left_upper_text)[0] + width//30
            draw.text((x, y), recorded_strongness, font=analog_small_font, fill=textColor)
            tmp = np.array(img_pil)
